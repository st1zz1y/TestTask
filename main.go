...
// initDB устанавливает соединение с базой данных и выполняет миграцию.
func initDB() {
...
func enrichData(name string) (age int, gender string, nationality string, err error) {
        ...
        if err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка вызова agify: %v", err)
        }
        ...
        if err = json.NewDecoder(resp.Body).Decode(&agifyResp); err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка декодирования ответа agify: %v", err)
        }
        age = agifyResp.Age
        ...
        if err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка вызова genderize: %v", err)
        }
        defer resp.Body.Close()
        ...
        if err = json.NewDecoder(resp.Body).Decode(&genderizeResp); err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка декодирования ответа genderize: %v", err)
        }
        ...
        if err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка вызова nationalize: %v", err)
        }
        defer resp.Body.Close()
        ...
        if err = json.NewDecoder(resp.Body).Decode(&nationalizeResp); err != nil {
                ...
                return 0, "", "", fmt.Errorf("ошибка декодирования ответа nationalize: %v", err)
        }
...
func createPerson(c *gin.Context) {
        ...
        if err != nil {
                c.JSON(http.StatusInternalServerError, gin.H{"error": "Ошибка обогащения данных: " + err.Error()})
                return
        }
        person := Person{
                Name:       input.Name,
...
func getPersons(c *gin.Context) {
        ...
        if err != nil || limit < 1 {
                c.JSON(http.StatusBadRequest, gin.H{"error": "Некорректное количество записей на странице"})
                return
        }
        offset := (page - 1) * limit
